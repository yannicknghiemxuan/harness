...
#
configname=$1
[[ -z $1 ]] && echo "usage: $0 <config_name>" && exit 1

# >>>>>>>>>>>>>>>>>>>>>> Start of Configuration >>>>>>>>>>>>>>>>>>>>>>
#
if [[ ! -f /galaxy/logic/config/namecheap/$configname ]]; then
    echo "error: /galaxy/logic/config/namecheap/$configname does not exist"
    exit 1
fi
. /galaxy/logic/config/namecheap/$configname
#
...
ipdir=/galaxy/var/inetip
[[ ! -d $ipdir ]] && mkdir -p $ipdir
...
do
  ...
  then
    ...
    saveIpFile=$ipdir/$CACHED_IP_FILE
...
then
  ...
  do
    ...
    then
      ...
      # ip=$(cat /galaxy/logic/config/namecheap/$configname)
      # oldIp=$(nslookup $host.$DOMAIN | grep -E '^Address' | tail -n 1 | awk '{print $2}')
      if [ "$oldIp" != "$ip" ]
      then
